...
parser.add_argument('--model', '-m', default='wrn-34-10', type=str, help='Name of the model: wrn-XX-XX, resnet-XX, small-cnn')
parser.add_argument('--test-batch-size', type=int, default=200, metavar='N',
                    help='input batch size for testing (default: 200)')
parser.add_argument('--no-cuda', action='store_true', default=False,
                    help='disables CUDA training')
parser.add_argument('--epsilon', type=float, default=12.0/255.0,
                    help='perturbation')
...
if args.dataset == 'cifar10':
    ...
    args.epsilon = 12.0 / 255.0
    args.step_size = 2.0 / 255.0
...
def main():
    ...
    if args.white_box_attack:
        ...
        #model = WideResNet(depth=34, num_classes=100 if args.dataset == 'cifar100' else 10).to(device)
        ...
        model = get_model(args.model, num_classes=10, normalize_input=normalize_input)
        if use_cuda:
        ...
        model_target = WideResNet(depth=34, num_classes=100 if args.dataset == 'cifar100' else 10).to(device)
        model_target.load_state_dict(torch.load(args.target_model_path))
        model_source = WideResNet(depth=34, num_classes=100 if args.dataset == 'cifar100' else 10).to(device)
